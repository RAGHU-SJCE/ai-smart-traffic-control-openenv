---
title: Rl Demo Environment Server
emoji: 🎬
colorFrom: green
colorTo: pink
sdk: docker
...
The simplest way to use the Rl Demo environment is through the `RlDemoEnv` class:
...
try:
    ...
    RL_demoenv = RlDemoEnv.from_docker_image("RL_demo-env:latest")
    ...
    for msg in messages:
        result = RL_demoenv.step(RlDemoAction(message=msg))
        print(f"Sent: '{msg}'")
...
That's it! The `RlDemoEnv.from_docker_image()` method handles:
